...
class RagPipelineOllamaQdrant(RagInterface):
    def __init__(
        ...
        ollama_url: str = "http://a6000.ml.c.com:11434",
        ollama_model: str = "qwen2.5:32b",
        embed_model: str = "mxbai-embed-large:latest",
    ):
        self._qdrant_client = QdrantClient(
            url=f"{qdrant_host}:{qdrant_port}", api_key=qdrant_api_key, timeout=60
        )
    ...
    def _check_connections(self):
        ...
        try:
            ...
            if self._collection_name in collection_names:
                ...
                logger.warning(
                    f"❌ Коллекция '{self._collection_name}' не найдена. Доступные: {collection_names}"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ Ollama подключен. Модели: {[m['name'] for m in models]}"
                )
            else:
